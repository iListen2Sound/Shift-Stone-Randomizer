	...
	internal class LoadoutInteractor
	{
		...
		{{
			...
			public Slot(Vector3 position, MelonPreferences_Entry<string> leftPref, MelonPreferences_Entry<string> rightPref, bool isSaveButton)
			{
				...
				_leftStoneSlot.transform.localPosition = new Vector3(0.08f, 0.07f, 0.01f);
				_leftStoneSlot.transform.localScale = new Vector3(0.3f, 0.3f, 0.3f);
				_leftStoneSlot.transform.SetParent(Button.transform, false);
				
				_rightStoneSlot.transform.localPosition = new Vector3(0.08f, 0.07f, -0.01f);
			...
			public DisplayShiftStones(ShiftStonePrefs left, ShiftStonePrefs right)
			{
...
#region static Members
		...
		public static ImmutableList<Vector3> Sections = new ImmutableList<Vector3>()
		{
			new Vector3(-0.04f, 0.04f, 0.07f),
			new Vector3(0.04f, 0.04f, 0.07f),
			new Vector3(-0.04f, -0.04f, 0.07f),
			new Vector3(0.04f, -0.04f, 0.07f),
		};
		...
		public GameObject Cluster { get {return _cluster;} } 
	...
	{
		public static ImmutableList<StoneItem> StoneHandlers = ImmutableList.Create
			(	
		...
		public static GetStoneItem(ShiftStonePrefs stoneEnum)
		{
			if((int) stoneEnum >= 0 || (int) stoneEnum <= 7)
			{
				return StoneHandlers[(int) stoneEnum];
			}
		}
	}
}
