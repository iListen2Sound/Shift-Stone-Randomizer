...
namespace Shift_Stone_Randomizer
{
    public class Class1 : MelonMod
    {
        ...
        private void CreatePhysicalGUI() 
        {
            ...
            {
                ToggleOneHanded(Calls.Managers.GetPlayerManager().LocalPlayer.Controller.GetComponent<PlayerShiftstoneSystem>().GetCurrentShiftStoneConfiguration());
            });
        }
        ...
        private void ToggleOneHanded(int[] Stones)
        {
            if (Stones[0] == -1)
            {
                EquipStones(-1, Stones[1], true, true);
                keepStone = 0;
            }
            if (Stones[1] == -1)
            {
                EquipStones(Stones[0], -1, true, true);
                keepStone = 1;
            }
